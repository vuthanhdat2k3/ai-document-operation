...
def make_reason_node(spec: AgentSpec) -> Any:
    ...
    async def reason_node(state: dict[str, Any]) -> dict[str, Any]:
        ...
        llm_result = _call_llm(filled_prompt, user_message, spec)
...
def _call_llm(system_prompt: str, user_message: str, spec: AgentSpec) -> dict[str, Any]:
    """Call the LLM and parse the JSON response.

    Uses the model configuration from the AgentSpec.
    Falls back to synthesize if the LLM is unavailable.
    """
    try:
        ...
        response = client.chat.completions.create(
            model=spec.model.model_name or settings.DEFAULT_MODEL,
            messages=[
